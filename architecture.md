...
`S4D` gives in Fig. 1 (`(exp(dt*A) - 1)/A`, which is exactly the ZOH `B̄` formula above, since
...
between the two families, not an oversight; it is called out explicitly in `experiments.md`
...
pre-norm, dropout, weight tying of `wte`/`lm_head`, optimizer (AdamW), LR schedule (warmup and
